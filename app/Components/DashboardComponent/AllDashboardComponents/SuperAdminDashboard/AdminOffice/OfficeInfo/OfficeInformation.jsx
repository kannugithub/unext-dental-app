...
const OfficeInformation = ({ closeOfficeinfo }) => {
  const [values, setValues] = useState({});
  const [errors, setErrors] = useState({});
  const [selectedImage, setSelectedImage] = useState(null);
  const [selectedFile, setSelectedFile] = useState(null);
  const [isDarkTheme, setIsDarkTheme] = useState(null);
  const [officethankyouPopup, setOfficethankyouPopup] = useState(false);
  ...
  const openOfficethankyou = (e) => {
    setOfficethankyouPopup(true);
  };
  const closeOfficethankyou = (e) => {
    setOfficethankyouPopup(false);
  };
  ...
  return (
    <>
        ...
        <div className={styles.addOffice_container}>
          <div className={styles.close_icons}>
            {isDarkTheme === "dark" ? (
              ...
              //   ></Image>
              //   <Image src={Images.whiteclose_icon}></Image>
              ""
            )}
          </div>
          <div className={styles.add_new_dental_box}>
            <div className={styles.add_new_list}>Dental Business Name</div>
            <div className={styles.add_form_box}>
              <div className="col-md-12">
                ...
                <div className={styles.office_box}>
                  ...
                  <input
                    type="text"
                    placeholder="1258 9874 8520 4569"
                    name="fax_Number"
                    maxLength={10}
            ...
            <div className={styles.add_new_dental_button}>
              <div className={styles.add_new_dental_btn}>
                <button>Save</button>
              </div>
              <div className={styles.add_new_dental_skip}>
                <button>Skip</button>
              </div>
            </div>
          </div>
      ...
      {officethankyouPopup && (
        <RightCustomModal
          isOpen={officethankyouPopup}
          onClose={closeOfficethankyou}
          width="40w"
          shouldCloseOnOutsideClick={true}
        >
          <OfficeThanyouPopup
            clinicId={clinicId}
            setOfficethankyouPopup={setOfficethankyouPopup}
            closeOfficethankyou={closeOfficethankyou}
          />
        </RightCustomModal>
      )}
    </>
