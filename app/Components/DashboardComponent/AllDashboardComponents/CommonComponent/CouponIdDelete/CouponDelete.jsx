import React, { useState, useEffect } from "react";
import { useDispatch, useSelector } from "react-redux";
import styles from "../CouponIdDelete/couponDelete.module.scss";
import {
  deleteCouponFromWeb,
  fetchDeleteSupportdata,
} from "@/app/store/slices/superAdminSlices";
import { toast } from "react-toastify";
function CouponDelete({
  cancelCouponDelete,
  deleteType,
  setCouponDeleted,
  selectedCouponId,
}) {
  const dispatch = useDispatch();
  // const isDarkTheme = useSelector((state) => state.darkTheme.isDarkTheme);
  const [isDarkTheme, setIsDarkTheme] = useState(null);
  const reduxTheme = useSelector((state) => state.darkTheme.isDarkTheme);

  useEffect(() => {
    if (typeof window !== "undefined") {
      setIsDarkTheme(reduxTheme ? "dark" : "light");
    }
  }, [reduxTheme]);

  const handleDeleteClinic = () => {
    if (deleteType === "cancelCouponDelete") {
      dispatch(deleteCouponFromWeb(cancelCouponDelete))
        .then((action) => {
          if (action.payload.success) {
            toast.success(action.payload.messsage || action.payload.message);
            setCouponDeleted(false);
          } else if (action.error) {
            toast.error(action.error.message);
            setCouponDeleted(false);
          }
        })
        .catch((error) => {
          toast.error(error.message || "Error deleting clinic");
        });
    } else if (deleteType === "Deleteticketdata") {
      dispatch(fetchDeleteSupportdata(selectedCouponId))
        .then((action) => {
          if (action.payload.success) {
            toast.success(action.payload.messsage || action.payload.message);
            setCouponDeleted(false);
          } else if (action.error) {
            toast.error(action.error.message);
            setCouponDeleted(false);
          }
        })
        .catch((error) => {
          toast.error(error.message || "Error deleting clinic");
        });
    }
  };

  return (
    <div>
      {" "}
      <div
        className={
          isDarkTheme === "dark" ? styles.darkHeader : styles.lightHeader
        }
      >
        <div className={styles.confirm_container}>
          <div className={styles.cross_img}>
            <svg
              width="95"
              height="95"
              viewBox="0 0 95 95"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M47.0551 0.734375C21.5425 0.734375 0.735352 21.5415 0.735352 47.0541C0.735352 72.5668 21.5425 93.3739 47.0551 93.3739C72.5677 93.3739 93.3749 72.5668 93.3749 47.0541C93.3749 21.5415 72.5677 0.734375 47.0551 0.734375ZM47.0551 87.492C24.7775 87.492 6.61723 69.3317 6.61723 47.0541C6.61723 24.7765 24.7775 6.61625 47.0551 6.61625C69.3327 6.61625 87.493 24.7765 87.493 47.0541C87.493 69.3317 69.3327 87.492 47.0551 87.492Z"
                fill="#409EEE"
              />
              <path
                d="M49.9961 40.0703H44.1143V69.4797H49.9961V40.0703Z"
                fill="#409EEE"
              />
              <path
                d="M49.9961 26.0996H44.1143V31.9815H49.9961V26.0996Z"
                fill="#409EEE"
              />
            </svg>
          </div>
          <div className={styles.title_text}>
            <h1>Are you sure?</h1>
          </div>
          <div className={styles.confirm_hTag}>
            Coupon with ID #852145 will be delete
          </div>
          <div className={styles.confirm_btns}>
            <button onClick={() => setCouponDeleted(false)}>cancel</button>
            <button onClick={handleDeleteClinic}>delete</button>
          </div>
        </div>
      </div>
    </div>
  );
}

export default CouponDelete;
